...
/*
    ...
    async function getProject2() {
        ...
        try {
            ...
            console.log(projectWithId);
            console.log(parseInt(projectWithId.raisedAmount));
            console.log(projectWithId.donationCount);
        } catch (error) {
            console.error("Error:", error);
        }
    }
    ...
    async function getProjectDonations() {
        ...
        try {
            ...
            const projectWithId = await thisContract.getProjectDonations(2);
            console.log(projectWithId);
            // console.log(parseInt(projectWithId.raisedAmount));
            // console.log(projectWithId.donationCount);
        } catch (error) {
...
// console.log(`Balance: ${balance.toString()}`);
    ...
    return (
        ...
        <div className='bg-white p-6 rounded-lg border-solid border-2 border-coffee_5'>
            ...
            <button onClick={getProjectDonations} className="bg-white text-center mt-12 hover:text-white hover:bg-lime-900 hover:border-lime-900 font-semibold text-lime-900 py-2 px-4 border-2 border-lime-900 rounded-lg">donate</button>
            
            {/* {project.donations && <p>{ project.donations.length }</p>} */}
            </div>
        </div>}
        </div>
    );
